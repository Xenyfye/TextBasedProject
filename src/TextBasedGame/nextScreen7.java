...
public class nextScreen7 extends JPanel implements ActionListener{
	JFrame window = new JFrame();
	JLabel picture = new JLabel();
	JPanel backgroundPanel = new JPanel();
	private final JPanel btnPanel = new JPanel();
	private final JButton btnOne = new JButton("Explore -->");
	private final JButton btnTwo = new JButton("<-- Explore");
	JButton doorBtn = new JButton("Open doors");

	nextScreen7(Inventory testInv){
		btnPanel.setBackground(Color.BLACK);
	
		JTextArea textArea = new JTextArea();
		textArea.setWrapStyleWord(true);
		textArea.setEditable(false);
		textArea.setText("\n\nGather more information on how to properly get out or make a break for it");
		...
		doorBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(testInv.checkForItem("Master Key")) {
					endScreen finalScreen = new endScreen(testInv);
					window.dispose();
				} else {
						JOptionPane.showMessageDialog(window.getContentPane(), "You need a Master Key to access this room");
						JOptionPane.showMessageDialog(window.getContentPane(), testInv.toString());
				}
		...
		btnTwo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				nextScreen10 next = new nextScreen10(testInv);
				window.dispose();
			}
		});
		btnOne.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(testInv.checkForItem("Master Key")) {
					nextScreen9 next = new nextScreen9(testInv);
					window.dispose();
				} else { 
					JOptionPane.showMessageDialog(window.getContentPane(), "You need a Master Key to access this room");
				}
		...
		picture.setIcon(new ImageIcon("C:\\Users\\opadm\\eclipse-workspace\\CSIS 2450_TeamProject\\media\\UpstairsDoors.png"));
